...
class RecipeCreateRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('recipes.create');
    }
    ...
    public function rules(): array
    {
        ...
        return [
            'product_id' => [
                'nullable', // Changed from 'required' to 'nullable' - product link is optional
                'uuid',
                'exists:products,id,tenant_id,' . $tenantId . ',deleted_at,NULL'
            ],
            'name' => 'required|string|max:255',
            'description' => 'nullable|string',
            'yield_quantity' => 'required|numeric|min:0.001',
            'yield_unit' => 'required|in:pcs,kg,L,serving,batch',
