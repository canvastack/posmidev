...
return new class extends Migration {
    public function up(): void
    {
        ...
        Schema::table('roles', function (Blueprint $table) use ($isSqlite) {
            ...
            // Drop existing unique on (name, guard_name) if exists (only on non-sqlite), then add new
            if (!$isSqlite) {
        ...
        if (!$isSqlite) {
            // Ensure tenant_id types are UUID even if created as BIGINT by earlier tables (Postgres-only)
    ...
    public function down(): void
    {
        ...
        Schema::table('model_has_roles', function (Blueprint $table) use ($isSqlite) {
        ...
        Schema::table('model_has_permissions', function (Blueprint $table) use ($isSqlite) {
            // Postgres-safe: drop constraint/index only if exists
            if (!$isSqlite) {
